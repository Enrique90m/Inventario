...
namespace Facturacion
{
    public partial class Factura : Form, EnlaceEntreFormularios
    {
        public Factura(string usuario)
        {
            ...
            this.Text = "Sistema de Facturacion : " + usuario;
        ...
        public void AgregaFactura()
        {
            ...
            if (actualiza == true)
            {
                int folio = int.Parse(DatosCliente_Folio.Text);
                this.BuscarAlgoenTabla("SELECT * FROM Factura WHERE Num_Factura = "+folio.ToString());

                if (dt.Rows.Count == 0)
                {
                    MessageBox.Show("No se encontro tal factura", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                    renglon = 0;
                    this.ponedatos(false);
                    return;
                }
               ...
               // dt.Rows[folio]["Num_Factura"] = dt.Rows.Count + 1;
                dt.Rows[0]["Cliente"] = DatosCliente_cliente.Text;
                dt.Rows[0]["Direccion"] = DatosCliente_direccion.Text;
                dt.Rows[0]["Localidad"] = DatosCliente_localidad.Text;

                if (!string.IsNullOrEmpty(DatosClienteIdcliente.Text) && !string.IsNullOrWhiteSpace(DatosClienteIdcliente.Text))
                    dt.Rows[0]["id_Socio"] = long.Parse(DatosClienteIdcliente.Text);

                if (!string.IsNullOrEmpty(DatosCliente_DNI.Text) && !string.IsNullOrWhiteSpace(DatosCliente_DNI.Text))
                    dt.Rows[0]["DNI"] = long.Parse(DatosCliente_DNI.Text);

                dt.Rows[0]["Direccion"] = !string.IsNullOrEmpty(DatosCliente_direccion.Text) && !string.IsNullOrWhiteSpace(DatosCliente_direccion.Text) ? DatosCliente_direccion.Text : "";
                dt.Rows[0]["Localidad"] = !string.IsNullOrEmpty(DatosCliente_localidad.Text) && !string.IsNullOrWhiteSpace(DatosCliente_localidad.Text) ? DatosCliente_localidad.Text : "";

                if (!string.IsNullOrEmpty(DatosCliente_cp.Text) && !string.IsNullOrWhiteSpace(DatosCliente_cp.Text))
                    dt.Rows[0]["Cp"] = long.Parse(DatosCliente_cp.Text);

                dt.Rows[0]["Provincia"] = !string.IsNullOrEmpty(DatosCliente_provincia.Text) && !string.IsNullOrWhiteSpace(DatosCliente_provincia.Text) ? DatosCliente_provincia.Text : "";
                dt.Rows[0]["Pais"] = !string.IsNullOrEmpty(DatosCliente_pais.Text) && !string.IsNullOrWhiteSpace(DatosCliente_pais.Text) ? DatosCliente_pais.Text : "";
                dt.Rows[0]["Fecha"] = Fecha.Text;

                dt.Rows[0]["Subtotal"] = double.Parse(Facturacion_textbox_subtotal.Text);
                dt.Rows[0]["Iva"] = double.Parse(Facturacion_textbox_iva.Text);
                dt.Rows[0]["Total"] = double.Parse(Facturacion_textbox_Total.Text);
                ...
                for (int i = 0; i < Facturacion_DatagridArticulos.Rows.Count; i++)
                {
                    if (Facturacion_DatagridArticulos.Rows[i].Cells[0].Value.ToString() != "")
                        dt.Rows[0]["Codigo" + (i + 1)] = Facturacion_DatagridArticulos.Rows[i].Cells[0].Value.ToString();

                    if (Facturacion_DatagridArticulos.Rows[i].Cells[1].Value.ToString() != "")
                        dt.Rows[0]["Cantidad" + (i + 1)] = Facturacion_DatagridArticulos.Rows[i].Cells[1].Value.ToString();

                    dt.Rows[0]["Descripcion" + (i + 1)] = Facturacion_DatagridArticulos.Rows[i].Cells[2].Value.ToString();
                    dt.Rows[0]["PrecioUnitario" + (i + 1)] = Facturacion_DatagridArticulos.Rows[i].Cells[3].Value.ToString();
                }
                //TOTAL DE ARTICULOS COMPRADOS
                dt.Rows[0]["TotalDatos"] = Facturacion_DatagridArticulos.Rows.Count;
        ...
        public void impresion(StreamWriter sw)
        {
            ...
            sw.WriteLine("                (N.19) Sucursal : Apodaca");
            sw.WriteLine("                   Avenida Miguel aleman #19");
            sw.WriteLine("                    Apodaca N.L. Mexico");
            sw.WriteLine("\n");
            sw.WriteLine("-------------------------Datos del cliente -----------------------");
            sw.WriteLine("Cliente: " + DatosCliente_cliente.Text + "        Localidad : "+DatosCliente_localidad.Text);
            sw.WriteLine("DNI: " + DatosCliente_DNI.Text+"                               CP: "+ DatosCliente_cp.Text);
            sw.WriteLine("Direccion: " + DatosCliente_direccion.Text+"        Provincia: "+ DatosCliente_provincia.Text);
            sw.WriteLine("Pais: "+DatosCliente_pais.Text);
            ...
            sw.WriteLine("***********************************************************************");
            sw.WriteLine("                         Subtotal: $" + Facturacion_textbox_subtotal.Text);
            sw.WriteLine("                         Iva : $" + Facturacion_textbox_iva.Text);
        ...
        private void Facturacion_boton_imprimir_Click(object sender, EventArgs e)
        {
             ...
             */
            StreamWriter sw = new StreamWriter("C:\\Facturacion\\Facturacion\\Factura.txt");
            impresion(sw);
            sw.Close();
            MessageBox.Show("Factura virtual creada!");
        ...
        {
            if (string.IsNullOrEmpty(DatosClienteIdcliente.Text) || string.IsNullOrWhiteSpace(DatosClienteIdcliente.Text) || DatosClienteIdcliente.Enabled==false || DatosClienteIdcliente.ReadOnly == true)
                return;
        ...
        private void DatosCliente_Folio_Leave(object sender, EventArgs e)
        {
            ...
            this.BuscarAlgoenTabla("SELECT * FROM Factura");            
            renglon = int.Parse(DatosCliente_Folio.Text);
            if (renglon == dt.Rows.Count - 1)
                renglon--;
            this.ponedatos(false);
        }
        ...
        {
            foreach (Control c in this.groupBox2.Controls)
                if (c is TextBox && c.Name!="Fecha" && !c.Name.Contains("Folio"))
                    c.Text = "";
        }

        private void button2_Click_1(object sender, EventArgs e)
        {
            ListaFactura list = new ListaFactura(dt,da,1);
            list.Show();
        }

        private void button3_Click(object sender, EventArgs e)
        {
            ListaFactura list = new ListaFactura(dt, da, 2);
            list.Show();
        }
        ...
        private void button4_Click_1(object sender, EventArgs e)
        {
            ...
            DialogResult respuesta = MessageBox.Show("Desea enviar factura? ", "", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            if (respuesta.Equals(DialogResult.No))
                return;

            //CREA ARCHIVO
            StreamWriter sw = new StreamWriter("C:\\Facturacion\\Facturacion\\Ticket.txt");
            impresion(sw);
            sw.Close();
          
            //LEE ARCHIVO
            StreamReader objReader = new StreamReader("C:\\Facturacion\\Facturacion\\Ticket.txt");
            string sLine = "";
            ...
            MessageBox.Show("Factura enviada correctamente!","",MessageBoxButtons.OK,MessageBoxIcon.Information);
        }
